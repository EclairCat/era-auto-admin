...
import { element, resetWarningCache } from 'prop-types';
...
class Commande_Client extends Component {
  ...
  */
  ...
  getAllOrder() {
    axios.get('http://localhost:4000/api/showBcClient',
    {
      params : {
  ...
  tableFiltre() {
    ...
    if (this.state.client_id != "" && this.state.client_id != null && newOrderFiltered.length != 0) {
      console.log("je passe name client")

      newOrderFiltered = newOrderFiltered.filter(e => e.Name.toLowerCase().includes(this.state.client_id.toLowerCase()));
      filtered = true;
    ...
    if (this.state.etat != null && this.state.etat != "" && newOrderFiltered.length != 0) {
      ...
      newOrderFiltered = newOrderFiltered.filter(e => e.etat == this.state.etat);
      filtered = true;
    ...
    if (this.state.date != null && this.state.date != "" && newOrderFiltered.length != 0) {
      ...
      newOrderFiltered = newOrderFiltered.filter(e => new Date(e.bc_date).getDate() == date.getDate() &&  new Date(e.bc_date).getMonth() == date.getMonth() && new Date(e.bc_date).getFullYear() == date.getFullYear());
      filtered = true;
    ...
    if (this.state.date_start != null && this.state.date_start != "" && this.state.date_end != null && this.state.date_end != "" && newOrderFiltered.length != 0) {
      ...
      newOrderFiltered = newOrderFiltered.filter(e => new Date(e.bc_date).getTime() >= start_date.getTime() && new Date(e.bc_date).getTime() <= end_date.getTime());
      filtered = true;
  ...
  cancelOrder(){
    ...
    axios.get('http://localhost:4000/api/cancelOrderId', {
      params: {
        id_order: this.state.id_bc_client_for_cancel,
        token : Cookies.get("auth_Era_Auto_Admin")
  ...
  renderCancelOrder(state, id){
    if(state != 4 && state != 3){
      return <Button color="danger" onClick={() => this.openModalCancelOrder(id)}> Cancel Order </Button>
    }
  }
        ...
        <Row>
          <Col>
            <Card>
              ...
              <Row>                
                <Col>
                <i className="fa fa-align-justify"> {t("Commande_Client.order_list")} </i>
                </Col>
                <Col sm={{ size: 'auto'}}>
                ...
                <Form>
                  <FormGroup>
                    <Row>
                      ...
                      <Col xs="4">
                        <Label>
                        {t("tables_info.states.state")} :
                        </Label>
                      
                        <Input type="select" name="etat" onChange={this.handleChange} id="select" value={this.state.etat}>
                          <option value=""></option>
                          <option value="0">{t("tables_info.states.open")}</option>                          
                          <option value="1">{t("tables_info.states.recieved")}</option>
                          <option value="2">{t("tables_info.states.in_progress")}</option>
                          <option value="3">{t("tables_info.states.closed")}</option>
                        </Input>
                ...
                <Table responsive striped>
                  ...
                  <tbody>
                    ...
                    {this.state.orders_sliced.map((value, index) => {
                      return <tr key={index}>
                        <td>{value.id_bc_client}</td>
                        <td>{new Date(value.bc_date).toLocaleDateString()}</td>
                        <td>{value.Name}</td>
                        <td>{value.total}</td>
                        <td>
                          {this.getOrderStatut(value.etat)}
                        </td>
                        <td>
                          <Button color="primary" onClick={() => this.openModal(value.id_bc_client)}> {t("tables_info.detail")} </Button>
                          {this.renderCancelOrder(value.etat, value.id_bc_client)}
                        </td>
                      </tr>
                    })}
