...
class Nouveau_Bl_Fournisseur extends Component {
  ...
  */
  ...
  getOrderByProviderId(id) {
    ...
    axios.get('http://localhost:4000/api/showBcIPClientByFrsId', {
      params: {
  ...
  */
  ...
  handleCheckboxChange(child, event) {
    ...
    let id = child.id_bc_ligne_client;

    new_ItemChecked[child.id_bc_ligne_client] = isChecked;
  ...
  */
  ...
  updateNewBcProvider() {
    ...
    clone_bc_client_by_provider.map(element => {
      ...
      if (this.state.itemChecked[element.id_bc_ligne_client]) {
  ...
  */
  ...
  createNewBcProvider() {
    ...
    if (this.state.new_bc_Provider.length != 0) {
      axios.post('http://localhost:4000/api/addBlFrs', {
        bc_provider: this.state.new_bc_Provider,
        id_provider: this.state.id_provider,
        token : Cookies.get("auth_Era_Auto_Admin")
        ...
        .then(res => {
          if (res.status = 200) {
            ...
            this.openModal("Success!", "The Chosen Products had been set to Received!")
      ...
      <div className="animated fadeIn">
        <Row>
          <Col>
            <Card>
              <CardHeader>
                <i className="fa fa-align-justify"></i> {t("Nouveau_Bl_Fournisseur.title")} 
              </CardHeader>
              <CardBody> 
                <Alert color="info" >
                  <Row>
                    <h2>{t("Nouveau_Bl_Fournisseur.info.title")} :</h2>
                  </Row>
                  <Row>
                  {t("Nouveau_Bl_Fournisseur.info.msg")}
                  </Row>
                </Alert>               
                <Alert color="danger" toggle={this.onDismiss} isOpen={this.state.alertBoxVisible}>
                {t("Nouveau_Bl_Fournisseur.select_provider_order")}
                </Alert>
                <Row>
                  <Label sm="2">
                  ...
                  <Col xs="4">
                    <Input type="select" name="id_provider" onChange={this.handleChange} value={this.state.id_provider} id="select">
                      <option value="none">
                      {t("Nouveau_Bl_Fournisseur.select_provider")}
                      </option>
                      {this.state.provider_list.map((value, index) => {
                        return <option value={value.id_provider} key={index}>
                          {value.name}
                        </option>
                      })}
                    </Input>
                  ...
                  <Table responsive striped>
                    <thead>
                      <tr>
                        ...
                        <th>{t("tables_info.id_client")}</th>
                        <th>{t("tables_info.name")}</th>
                    ...
                    <tbody>
                      ...
                      {this.state.bc_client_by_Id_Provider.map((value, index) => {
                        return <tr key={index}>
                          <td><input type="checkbox" checked={this.state.itemChecked[value.id_bc_ligne_client]} onChange={(e) => this.handleCheckboxChange(value, e)}></input></td>
                ...
                <Row>
                  <h2> {t("Nouveau_Bl_Fournisseur.new_table_title")} : </h2>
                  <Table responsive striped>
                    <thead>
                      <tr>
                        <th>{t("tables_info.product_name")}</th>
                ...
                <Button color="success" onClick={this.createNewBcProvider}> {t("Nouveau_Bl_Fournisseur.button")} </Button>
              </CardBody>
            </Card>
          </Col>
        </Row>
...
const Nouveau_Bl_Fournisseur_Translated = withTranslation('common')(Nouveau_Bl_Fournisseur)


export default Nouveau_Bl_Fournisseur_Translated;
